...
const AddToWishlistBtn = ({ product, slug }: AddToWishlistBtnProps) => {
  ...
  const addToWishlistFun = async () => {
    ...
    if (session?.user?.email) {
      ...
      fetch(`https://ecommerce-api-58ck.onrender.com/api/users/email/${session?.user?.email}`, {
        cache: "no-store",
      })
        .then((response) => response.json())
        .then((data) =>
          fetch("https://ecommerce-api-58ck.onrender.com/api/wishlist", {
            method: "POST",
            headers: {
  ...
  const removeFromWishlistFun = async () => {
    if (session?.user?.email) {
      ...
      fetch(`https://ecommerce-api-58ck.onrender.com/api/users/email/${session?.user?.email}`, {
        ...
        .then((data) => {
          return fetch(
            `https://ecommerce-api-58ck.onrender.com/api/wishlist/${data?.id}/${product?.id}`,
            {
              method: "DELETE",
            }
          );
